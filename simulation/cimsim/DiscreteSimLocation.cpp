...
typedef boost::numeric::converter<int,
                                  ...
                                  boost::numeric::RoundEven<double>> BankersRoundDoubleToInt;



DiscreteSimLocation::DiscreteSimLocation( const input::Location * location, boost::gregorian::date startDate, boost::gregorian::date stopDate, const sim::output::PopData * popData )
: _location(location),
...
DiscreteSimLocation::~DiscreteSimLocation(void)
{
  std::vector<DiscreteSimContainer*>::iterator itCont;
...
void
DiscreteSimLocation::AddContainer( input::Container * container )
{
  // create a DiscreteSimContainer from the input::Container optionally accounting for container cloning
  ...
  if( container->IsCloned_ ) {
    ...
    for( int cloneId = 1; cloneId <= container->NumberOfClones_; cloneId++ ) {
      DiscreteSimContainer * newContainer = new DiscreteSimContainer( container, _location->Biology_ );
...
void DiscreteSimLocation::AddPopContainer( input::Container * container, const sim::output::PopData * population )
{
  // create a DiscreteSimContainer from the input::Container optionally accounting for container cloning
  ...
  if( container->IsCloned_ ) {
    ...
    for( int cloneId = 1; cloneId <= container->NumberOfClones_; cloneId++ ) {
      ...
      DiscreteSimContainer * newContainer = new DiscreteSimContainer( container, _location->Biology_, cpd );
      newContainer->MakeClone( cloneId, container->NumberOfClones_, cloneDensity );
      _containers.push_back( newContainer );
    }
  }
...
void
DiscreteSimLocation::InitializePopulation( const sim::output::PopData * population )
{
  _usingPop = true;
...
void
DiscreteSimLocation::InitializeControls( input::ControlCollection controls )
{
...
void
DiscreteSimLocation::InitializeBiology( input::Biology * biology )
{
...
void
DiscreteSimLocation::RunSimulation(void)
{
...
void
DiscreteSimLocation::DoYear(void)
{
  ...
  for( Day = 1; Day <= numDays; ++Day, ++itDay) {
    ...
    for( itCont = _containers.begin(); itCont != _containers.end(); ++itCont ) {
      double densityAdjustedFemales = (*itCont)->CalculateDensityAdjustedNewFemaleCount();
      double roundedFemales = BankersRoundDoubleToInt::convert( densityAdjustedFemales );
      double densityAdjustedTotalWeight = (*itCont)->CalculateDensityAdjustedNewFemaleWeight();

      // new method
      sim::cs::DiscreteSimContainer * c = *itCont;
      double newFemales = c->GetNewFemaleCount();
      double productiveDensity = c->GetUntreatedDensity() + c->GetTreatedDensity();

      // scale and round new females from this container type, then use to calculate total weight
      newFemales = newFemales * productiveDensity;
      newFemales = BankersRoundDoubleToInt::convert( newFemales );
      double newFemaleWeight = newFemales * c->GetEmergedPupaeAverageWeight();

      NewFemales += newFemales;
      totalWeight += newFemaleWeight;
    }
    ...
    for( PreOviAdultIterator itAdult = _preOviAdults.begin(); itAdult != _preOviAdults.end(); ) {
      if( itAdult->Development < firstTargetThreshold || TemperatureAvg[Day] <= _minimumOvipositionTemperature ) {
        itAdult->Age++;
        itAdult->Number = StochasticAdvancement( itAdult->Number, AdultSurvival );
    ...
    for( PreOviAdultIterator itAdult = _nulliparousAdults.begin(); itAdult != _nulliparousAdults.end(); ) {
      ...
      itAdult->Number = StochasticAdvancement( itAdult->Number, survival );
    ...
    if( _totalOvipositingFemales > 0 ) {
      newEggs = BankersRoundDoubleToInt::convert( _totalOvipositingFemales * _fecundityCoefficient * ovipositingAverageWeight * sterileFactor );
    }
...
void
DiscreteSimLocation::CalculateAdultTemperatureSurvival(void)
{
...
void
DiscreteSimLocation::CalculateAdultSatDefSurvival(void)
{
...
double
DiscreteSimLocation::CalculateAdultAgeDependentSurvival( int age )
{
...
double
DiscreteSimLocation::CalculateDoubleBloodMealProportion( double weight )
{
...
void
DiscreteSimLocation::ReadWeather( int year )
{
...
void
DiscreteSimLocation::CalculateDailyTotals(void)
{
...
void
DiscreteSimLocation::UpdateOutput( date d, double adultDev, double newEggs )
{
...
DiscreteSimLocation::GetSimOutput(void)
{
...
sim::output::PopData *
DiscreteSimLocation::GeneratePopData(void)
{
...
bool
DiscreteSimLocation::IsEndOfMonth( boost::gregorian::date d )
{
...
double
DiscreteSimLocation::GetFemaleAverageWeight(void)
{
...
double
DiscreteSimLocation::GetOvipositingFemaleAverageWeight(void)
{
...
double
DiscreteSimLocation::GetNulliparousCount(void)
{
  double totalNumber = 0.0;
...
double
DiscreteSimLocation::GetParousCount(void)
{
  double totalNumber = 0.0;
...
DiscreteSimLocation::GetOvipositingTotals(void)
{
...
double
DiscreteSimLocation::GetNulliparousBiters(void)
{
...
double
DiscreteSimLocation::GetParousBiters(void)
{
