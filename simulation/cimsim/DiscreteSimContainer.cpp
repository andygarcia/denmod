...
extern int StochasticAdvancement( int number, double advancementFactor );



DiscreteSimContainer::DiscreteSimContainer( const input::Container * container, const input::Biology * bio, sim::output::ContainerPopData * containerPopData )
...
DiscreteSimContainer::~DiscreteSimContainer(void)
{
}
...
void
DiscreteSimContainer::MakeClone( int cloneId, int numOfClones, double newDensity )
{
...
void
DiscreteSimContainer::Initialize( date startDate )
{
...
void
DiscreteSimContainer::InitializeYear( int year )
{
...
void
DiscreteSimContainer::InitializeEggs(void)
{
...
void
DiscreteSimContainer::DoDay( boost::gregorian::date currentDate )
{
...
void
DiscreteSimContainer::CalculateWaterDepth( double todaysRain, double relHumid )
{
  double currentDepth;
...
void
DiscreteSimContainer::CalculateWaterTemperature( double minAirTemp, double avgAirTemp, double maxAirTemp )
{
  if( _waterDepth <= 0 ) {
...
double
DiscreteSimContainer::MinWaterTempRegression( double minAirTemp, double maxAirTemp )
{
  double minTemp;
...
double
DiscreteSimContainer::MaxWaterTempRegression( double minAirTemp, double maxAirTemp )
{
  double maxTemp;
...
void
DiscreteSimContainer::CalculateDevelopmentRates( int day )
{
...
void
DiscreteSimContainer::ApplyMonthlyDensityAdjustment(void)
{
...
void
DiscreteSimContainer::ApplySourceReduction( double rateOfReduction )
{
...
void
DiscreteSimContainer::RecoverExcluded(void)
{
...
double
DiscreteSimContainer::GetTotalExcludedDensity(void)
{
...
void
DiscreteSimContainer::ApplyOvipositionExclusion( double rateOfExclusion, double rateOfRecovery )
{
...
void
DiscreteSimContainer::RecoverTreated(void)
{
...
void
DiscreteSimContainer::BeginNewLarvicideTreatment( input::Larvicide * li )
{
  // find treatment rate
...
void
DiscreteSimContainer::ApplyEggDestruction( double rateOfDestruction )
{
...
void
DiscreteSimContainer::CalculateSaturationDeficitEggSurvival( int day, double satDef )
{
  if( _waterDepth == 0 ) {
...
void
DiscreteSimContainer::CalculateTemperatureEggSurvival( int day )
{
...
void
DiscreteSimContainer::CalculatePredationEggSurvival( int day )
{
...
void
DiscreteSimContainer::CalculateCumulativeEggSurvival(void)
{
...
void
DiscreteSimContainer::AdvanceEggs( int day )
{
  ...
  for( EggBandIterator itBand = EggCohorts.begin(); itBand != EggCohorts.end(); ++itBand ) {
    ...
    // process mature cohorts, if any
    if( eggBand->MatureEggs > 0 ) {
      eggBand->MatureEggs = StochasticAdvancement( eggBand->MatureEggs, _eggSurvival );
    }
    ...
    for( EggIterator itEgg = eggBand->EggCohorts.begin(); itEgg != eggBand->EggCohorts.end();  ) {
      if( itEgg->Development <= targetThreshold ) {
        itEgg->Age++;
        itEgg->Number = StochasticAdvancement( itEgg->Number, _eggSurvival );
        ...
        if( _averageWaterTemperature < _eggMinimumHatchTemperature ) {
          eggBand->MatureEggs = eggBand->MatureEggs + StochasticAdvancement( itEgg->Number, _eggSurvival );
        }
        else {
...
void
DiscreteSimContainer::ApplyDryContainerToNewLarvae( int day )
{
  if( _waterDepth == 0 ) {
    _newlyHatched = 0;
  }
...
void
DiscreteSimContainer::AdjustFood( date currentDate )
{
...
void
DiscreteSimContainer::CalculateTemperatureLarvalSurvival( int day )
{
...
void
DiscreteSimContainer::CalculateLarvicideLarvalSurvival( date d )
{
  if( _currentLarvicideTreatment != NULL ) {
    if( _currentLarvicideTreatment->InEffect ) {
...
void
DiscreteSimContainer::CalculateWaterDepthLarvalSurvival( int day )
{
...
void
DiscreteSimContainer::CalculateLarvalFoodConsumptionAndWeight( int day, int eulerSteps )
{
...
double DiscreteSimContainer::FoodEqn( double nt, double WT, double FT, double et )
{
...
double DiscreteSimContainer::WeightEqn( double WT, double FT, double et )
{
// Larval weight gain
//
// FT     :weight of food within the container in terms of food
...
void
DiscreteSimContainer::AdvanceLarvae( int day )
{
  ...
  for( LarvaeIterator itLarvae = LarvaeCohorts.begin(); itLarvae != LarvaeCohorts.end(); ) {
    ...
    if( itLarvae->Number == 0 ) {
      ...
      if( itLarvae->Development <= targetThreshold || itLarvae->Weight < PupationWt ) {
        ...
        // advance cohort
        double initNumber = itLarvae->Number;
        double initWeight = itLarvae->Weight;
        double survival = _larvaeNominalSurvival * _larvaeTemperatureSurvival * _larvaeLarvicideSurvival * itLarvae->SurvivalFood;

        // advance in age
        itLarvae->Age++;

        // accumulate development and weight change
        itLarvae->Development = itLarvae->Development + _larvaeDevRate;
        itLarvae->Weight = itLarvae->Weight + itLarvae->WeightChange;

        // discard if below threshold weight or have reached max development
        if( itLarvae->Weight < _larvaeMinimumWeightForSurvival  || itLarvae->Development > _larvaeMaximumDevelopment ) {
          totalCadaverWeight += initNumber * initWeight;
          itLarvae = LarvaeCohorts.erase( itLarvae );
          continue;
        }

        // track total larvae and death from fasting (negative weight change)
        if( itLarvae->WeightChange < 0 ) {
          _totalLarvaeFasting += initNumber * survival;
          _totalLarvaeDeathFromFasting += initNumber * (1 - itLarvae->SurvivalFood);
        }

        // apply survival and calculate cadavers
        double survivingLarvae = StochasticAdvancement( itLarvae->Number, survival );
        double deadLarvae = itLarvae->Number - survivingLarvae;
        ...
        // advance surviving larvae and track weight
        newPupae += survivingLarvae;
        totalNewPupaeWeight += itLarvae->Weight * (survivingLarvae);

        // dead larvae accumulate as cadavers
        totalCadaverWeight += deadLarvae * weight;
...
void
DiscreteSimContainer::ApplyGeneticPupationDeath( int day )
{
  double geneticPupationSurviving = StochasticAdvancement( _newPupae, _larvaePupationSurvival );
  double geneticPupationDeath = _newPupae - geneticPupationSurviving;

  _newPupae = geneticPupationSurviving;
  _pupaeCadaverWeight = geneticPupationDeath * _larvaeMaxWeight;
}



void
DiscreteSimContainer::HatchEggs( int day )
{
  if( _averageWaterTemperature >= _eggMinimumHatchTemperature ) {
    for( int i = _hatchBand; i >= 1; --i ) {
      EggBand * eggBand = &EggCohorts[i];

      double hatchedEggs = StochasticAdvancement( eggBand->MatureEggs, _eggFloodHatchRatio );
...
void
DiscreteSimContainer::CalculateTemperaturePupalSurvival( int day )
{
...
void
DiscreteSimContainer::AdvancePupae(void)
{
  ...
  for( PupaeIterator itPupae = PupaeCohorts.begin(); itPupae != PupaeCohorts.end(); ) {
    // apply survival
    double survivingPupae = StochasticAdvancement( itPupae->Number, _pupaeTemperatureSurvival * _pupaeNominalSurvival );
    ...
    if( itPupae->Development <= targetThreshold ) {
      ...
      double emergedPupae = StochasticAdvancement( survivingPupae, _pupaeEmergenceSuccess );
...
void
DiscreteSimContainer::SelectFemales(void)
{
  _newFemales = StochasticAdvancement( _newAdults, _pupaeFemaleEmergenceRatio );
}



double
DiscreteSimContainer::CalculateDensityAdjustedNewFemaleCount(void)
{
...
double
DiscreteSimContainer::CalculateDensityAdjustedNewFemaleWeight(void)
{
...
double
DiscreteSimContainer::GetNewFemaleCount(void)
{
  return _newFemales;
}
...
double
DiscreteSimContainer::GetEmergedPupaeAverageWeight(void)
{
  return _pupaeWeight;
}
...
double
DiscreteSimContainer::GetUntreatedDensity(void)
{
...
double
DiscreteSimContainer::GetTreatedDensity(void)
{
...
bool
DiscreteSimContainer::IsDry(void)
{
  if( _waterDepth > 0 )
    return false;
  else
...
double
DiscreteSimContainer::GetWaterVolume(void)
{
  double volume = 0.0;
...
double
DiscreteSimContainer::GetCapacityVolume(void)
{
  double volume = 0.0;
...
double
DiscreteSimContainer::GetSurfaceArea(void)
{
  double surfaceArea;
...
void
DiscreteSimContainer::ResetContPref(void)
{
...
double
DiscreteSimContainer::CalculateContPref( int day )
{
...
void
DiscreteSimContainer::NormalizeContPref( double normTotal )
{
...
double
DiscreteSimContainer::GetOvipositionPreference(void)
{
  return _ovipositionPreference;
}
...
void
DiscreteSimContainer::CalculateEggInputLevel( int day )
{
  _ovipositionBand = static_cast<int>( ceil(_waterDepth / _eggBandWidth));
...
double
DiscreteSimContainer::DistributeNewEggs( double totalNewEggs )
{
  // no eggs actually oviposited
  if( totalNewEggs <= 0 ) {
    _newEggCount = 0;
    return _newEggCount;
  }
  ...
  if( _untreatedDensity + _treatedDensity <= 0 ) {
    ...
    _newEggCount = StochasticAdvancement( totalNewEggs / (_untreatedDensity + _treatedDensity ), _ovipositionPreference );
    double oldNewEggCount = totalNewEggs * _ovipositionPreference / (_untreatedDensity + _treatedDensity);
    ...
    if( _ovipositionBand == _maxEggBand ) {
      ...
      // stochasticly distribute egg between top and bottom bands
      int bottomEggs = StochasticAdvancement( _newEggCount, .5 );
      int topEggs = _newEggCount - bottomEggs;

      EggCohort bottomCohort = EggCohort( 1, bottomEggs, 0 );
      EggCohort topCohort = EggCohort( 1, topEggs, 0 );

      EggCohorts[_ovipositionBand].EggCohorts.push_back( bottomCohort );
      EggCohorts[_ovipositionBand + 1].EggCohorts.push_back( topCohort );
    }
  }
...
DiscreteSimContainer::GeneratePopData(void)
{
...
void
DiscreteSimContainer::CalcDailyTotals()
{
  // tally eggs
...
DiscreteSimContainer::GetOutput( boost::gregorian::date d )
{
...
void
DiscreteSimContainer::EndDay(void)
{
  _eggDestructionSurvival = 1;
...
void
DiscreteSimContainer::EndYear(void)
{
