...
DiscreteSimulation::DiscreteSimulation( input::Location * location, boost::gregorian::date startDate, boost::gregorian::date stopDate, bool usePop )
  ...
  _usePop(usePop)
{
  if( _usePop ) {
    ...
    DiscreteSimLocation * popSim = new DiscreteSimLocation( location, popStartDate, popStopDate );
  ...
  SimLocation_ = new DiscreteSimLocation( location, startDate, stopDate, _popData );
}



DiscreteSimulation::~DiscreteSimulation(void)
{
  delete SimLocation_;
  if( _popData ) {
    delete _popData;
...
DiscreteSimulation::GetSimOutput(void)
{
...
FoodFitDiscreteSimulation::FoodFitDiscreteSimulation( input::Location * location, boost::gregorian::date surveyStartDate, boost::gregorian::date surveyStopDate )
: _location(location)
{
...
FoodFitDiscreteSimulation::~FoodFitDiscreteSimulation(void)
{
  delete _popData;
}
...
FoodFitDiscreteSimulation::DoIteration( int numberOfRuns )
{
  ...
  for( int i = 0; i < numberOfRuns; ++i ) {
    ...
    DiscreteSimLocation * foodRun = new DiscreteSimLocation( _location, _startDate, _stopDate, _popData );
